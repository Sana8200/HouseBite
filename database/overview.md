...
| `family_member` | A user linked 1:1 to `auth.users`. Stores name and join date. |
...
| `product_specs` | Size, quantity, expiry date, and price for a product. |
...
Every query automatically gets an invisible `WHERE` clause added by the policy. If a row doesn't pass the check, it is silently excluded from results.
...
CREATE FUNCTION my_households()
  RETURNS the list of household UUIDs the current user belongs to
```
...
In case of ever needing to re-set and rerun the DB, it is always importand to drop all existing tables, views and types created to avoid errors. 
For example (as of the current version - version 1):
